...
Demo UI `2.4.0`은 actual Phase 5.1 diagnosis를 sanitized static artifact로 재생합니다. 기록된 tool-call 순서, normalized evidence 요약, supporting/conflicting citation, evidence gap, deterministic validator 경계와 read-only 권한을 표시하며 OpenAI API를 다시 호출하지 않습니다. Public demo-lite는 image `7489ab270995`로 같은 static replay를 제공하고 recorded `local-ha` profile과 현재 저사양 runtime을 분리합니다.
...
| verified incident public replay | demo-lite UI `2.4.0` 배포 완료; static sanitized artifact만 공개하고 raw bundle·OpenAI 재호출 제외 |
