...
| 대상 | 확인된 버전·상태 | 증거 경계 |
| --- | --- | --- |
| `master` source | UI `2.3.1`, API `2.1.0`, tests `354 passed` | notification batch persistence와 Worker DB roundtrip 축소; clean fixed/KEDA 각 3회 검증 |
...
| dev GitOps release | image `8d334b8abeaf`, UI `2.3.1`, API `2.1.0` | `dev-kafka` CI validate·publish 통과 |
| public demo-lite runtime | UI `2.3.1`, API `2.1.0`, image `8640ca010960` | 2026-08-10 release 뒤 readiness `ready`, generic v2·`202`, Worker KEDA `1→2` |
| `demo-dev` profile | tests `358 passed`, Kafka `1`, PostgreSQL `1`, API·core Worker `1→2` | notification Worker `1`, 7일 storage retention을 유지한 저사양 GitOps deployment |
...
| --- | --- | --- |
| `master` source | UI `2.3.1`, API `2.1.0` | generic v2 + `202`; notification batch와 Worker DB roundtrip 최적화 포함 |
...
| dev GitOps release | UI `2.3.1`, API `2.1.0`, image `8d334b8abeaf` | CI validation 뒤 게시된 `dev-kafka` GHCR image |
| public demo-lite, 2026-08-10 | UI `2.3.1`, API `2.1.0`, image `8640ca010960` | release branch 갱신 뒤 readiness `ready`, generic v2 + `202`, KEDA `1→2` |
