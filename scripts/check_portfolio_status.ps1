...
if (-not $SkipPrometheus) {
  ...
  $consumerLag = Get-ScalarPrometheusValue "sum(clamp_min(kafka_consumergroup_lag{consumergroup=`"message-worker`"}, 0))"
  $notificationConsumerLag = Get-ScalarPrometheusValue "sum(clamp_min(kafka_consumergroup_lag{consumergroup=`"notification-worker`"}, 0))"
