...
$stream = Invoke-RestMethod -Method Post -Uri "$BaseUrl/v1/streams" -Headers @{ Authorization = "Bearer $u1Token" } -ContentType "application/json" -Body (@{ name = "dlq-stream-$suffix"; member_ids = @($u1.id, $u2.id) } | ConvertTo-Json)
...
try {
  ...
  $accept = Invoke-RestMethod -Method Post -Uri ("$BaseUrl/v1/streams/{0}/events" -f $stream.id) -Headers @{ Authorization = "Bearer $u1Token"; "X-Idempotency-Key"="dlq-$suffix"} -ContentType "application/json" -Body (@{ body = "force dlq while db down" } | ConvertTo-Json)
