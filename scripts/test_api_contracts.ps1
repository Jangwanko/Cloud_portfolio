...
try {
  ...
  $operations = Invoke-RestMethod -Method Get -Uri "$BaseUrl/ops/summary" -TimeoutSec 5
  Assert-HasProperty $operations "worker" "operations"
  Assert-HasProperty $operations.worker "available_replicas" "operations.worker"
  Assert-HasProperty $operations.worker "max_replicas" "operations.worker"
