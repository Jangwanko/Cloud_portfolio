...
| `demo-dev` candidate | Demo UI `2.1.0`, API `2.0.0`, generic v2, 저사양 overlay |
| public `demo-lite` | UI `1.4.1`, API `1.0.0`, image `e481a21`, generic v2 없음, event `200` |

`demo-dev` candidate는 아직 공개 서버의 상태가 아닙니다. 공개 URL이 새 image와 GitOps revision으로 전환되고 UI badge, readiness `app_version`, generic event `202`를 확인한 뒤 public 상태를 갱신합니다.
