...
| `Kafka Broker Availability` | `min(kafka_brokers)` instant query | 현재 연결 가능한 broker가 하나 이상인지 표시. demo-lite 기대값 `1`, full local-ha 기대값 `3` |
...
- PostgreSQL standby 수: 하단 상세 패널에서 확인. demo-lite의 정상값은 `0`, full local-ha의 정상값은 `2`
