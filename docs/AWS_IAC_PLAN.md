...
- `Amazon EKS`
  ...
  - HPA, Ingress, metrics 구조를 현재와 비슷하게 유지
- `Managed Node Group`
...
이 저장소에 실제 IaC를 추가한다면 아래 구조가 무난합니다.
...
- 애플리케이션 레벨의 `async intake`, `retry`, `DLQ`, `replayer`, `HPA`, `metrics`는 그대로 유지
...
즉 이 문서는 구현 전 아이디어 메모가 아니라, 실제 IaC 골격이 들어간 뒤 그 기준과 의도를 설명하는 문서입니다.
