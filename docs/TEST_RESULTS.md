...
| API Kafka intake | 통과 | API가 PostgreSQL 선행 write 없이 Kafka append-first path로 event를 수락 |
...
| Cache-first / degraded message read | 통과 | `scripts/test_cache_read_fallback.ps1`로 fresh cache read와 DB down stale cache fallback 확인 |
...
4. Worker가 Kafka event를 consume한 뒤 PostgreSQL에 message를 commit합니다.
...
| --- | --- |
| API readiness | `ready` |
| Argo CD Application | `Synced / Healthy`, revision `516f65fdebc5e244332fc8c02839563acb561afe` |
