...
배포 상태는 [Deployment Status](DEPLOYMENT_STATUS.md)와 관측 JSON을 근거로 갱신합니다. 이 유지보수 변경은 dev-kafka에서 구현하고 사용자 요청으로 master에 통합했습니다. Demo 브랜치 전파와 실제 runtime 승격은 별도 요청 범위입니다.
