...
- 데모 화면 버전 `1.3.4` 적용.
- 운영상태 자동 새로고침 기본값을 `10초`에서 `30초`로 변경.
- 브라우저 메모리에 demo auth token을 캐시해 자동 새로고침마다 `/v1/auth/login`을 호출하지 않도록 변경.
- DLQ summary 조회가 `401`을 반환할 때만 토큰을 강제 갱신.

해석:

- `/health/ready`는 가볍지만 `/v1/auth/login`은 users table 조회를 수행합니다.
- demo-lite에서 화면을 켜두기만 해도 10초마다 DB 인증 트랜잭션이 반복되면 Pgpool slot 압박으로 이어질 수 있습니다.
- 운영상태 확인은 보조 신호이므로, 자동 refresh는 느리게 두고 인증은 재사용합니다.
