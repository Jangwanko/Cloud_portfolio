...
- 2026-07-21 push 전 source 개선: `dev-kafka` image publication을 `ci.yml`의 `publish-dev-kafka-image` job으로 통합하고 `needs: validate`, exact tested SHA candidate digest 검증, branch advance guard를 적용했습니다. 아직 commit/push와 remote Actions run이 없으므로 배포 완료 증거로 사용하지 않습니다.
