...
- Phase 3.2 controlled Scenario Lab은 기존 adaptive loop와 v1을 유지하고 `ops.diagnosis.v2`를 별도 추가합니다. Actual Phase 5.1 `PRESENT` activation을 고정한 네 normalized fixture에서 observation-conditioned next-tool branching, acquisition provenance, Worker capacity/PostgreSQL path hypothesis, branch failure 보존을 검증합니다. `SCENARIO_LAB_ENABLED=true` local-ha의 `/admin/scenario-lab/`은 recorded artifact만 재생하며 OpenAI API와 runtime source를 호출하지 않습니다. `LIVE_READ_ONLY`는 provenance contract와 registry interface만 있고 connector는 미구현입니다.
...
- `demo-dev` source candidate는 Demo UI `2.5.0`, API `2.1.0`, commit `a67f40e`입니다. 네 controlled Scenario Lab 결과를 `demo.verified-scenario-replays.v1` public-safe artifact로 투영하고 첫 화면에서 condition → tool selection → observation → next branch → hypothesis → validator 순서로 재생합니다. focused `59 passed`, full suite `363 passed`, compileall·k6 inspect·artifact projection·secret/path/diff 검증을 통과했습니다. 이 값은 source candidate이며 public demo-lite 배포 증거가 아닙니다.
...
- `demo-dev`는 public demo-lite의 저사양 자원 경계를 관리합니다. Kafka append·DB persistence 동시 진행률, 운영 상태 패널의 Worker 현재/최대 replica, compact DB 저장 증거, 진행 중 Advisor 판정, migration → Worker → API gate를 포함합니다. UI `2.5.0` controlled scenario replay는 같은 시점의 Deployment current/ready/available과 KEDA current를 서로 다른 관측 의미로 표시하며 시간 순서나 scale-out 완료를 임의로 추론하지 않습니다.
...
- `demo-dev` candidate의 source Demo UI는 `2.5.0`입니다. 기존 저사양 event-processing UI와 actual Phase 5.1 replay를 유지하고, 네 controlled scenario의 recorded branching을 public-safe static artifact에서 재생합니다. 현재 public demo-lite는 UI `2.4.1`이며 이 후보는 아직 배포되지 않았습니다.
