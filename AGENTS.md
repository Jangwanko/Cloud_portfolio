...
- `demo-dev`에는 저사양 자원 경계를 유지한 generic v2 동기화 candidate가 있습니다. source Demo UI `2.1.0`, API `2.0.0`, migration → Worker → API gate를 포함합니다. CI는 검증된 `demo-dev` tree와 image tag를 단일 release commit으로 `demo-lite`에 게시하며 public 배포 완료 전까지 live 상태로 표현하지 않습니다.
- 2026-07-27 demo-lite candidate source 검증은 `368 passed`, 두 overlay render 성공, container build와 non-root UID `10001` 확인입니다. public k3s rollout 증거는 아직 없습니다.
...
- `demo-dev` candidate의 source Demo UI는 `2.1.0`입니다. 범용 envelope 증거가 Pipeline Evidence 안에 표시되며 public `demo-lite`로 배포되기 전에는 공개 화면 버전으로 기록하지 않습니다.
