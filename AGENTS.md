...
- 마지막 승격 기준 브랜치는 `master`입니다. 2026-08-05 운영 본체 단순화는 master merge `cab7647`로 승격했습니다. 2026-08-10 notification batch 최적화는 `dev-kafka` local candidate이며 아직 commit·publication 전입니다.
...
- 2026-08-10 notification batch 최적화 candidate는 local suite `354 passed`, image `messaging-portfolio:notification-batch`, clean 64-stream fixed/KEDA 각 3회 완료 상태입니다. commit·registry publication·GitOps release 전입니다.
