...
- Local HA topology: Kafka `3`, PostgreSQL `3`, Pgpool `2`, API min `6`, Worker min `2`, DLQ replayer `1`.
...
- 2026-07-27 demo-lite generic v2 동기화 candidate의 local suite는 `368 passed`입니다.
...
- Public demo-lite는 2026-07-27 live GET 기준 title `Reliable Event Processing Console`, UI `2.1.0`, API `2.0.0`, generic v2 route와 event `202` 계약이 반영된 branch/deployment 전용 상태입니다. 같은 날 Prometheus range query에서 `message-worker` lag peak `828`, HPA desired와 실제 Worker replica `1→2`를 확인했습니다. Source UI `2.2.0`의 Kafka append·DB persistence 동시 진행률과 실행 중 Worker peak 표시는 아직 미배포입니다.
- `demo-dev`에는 저사양 자원 경계를 유지한 UI `2.2.0` candidate가 있습니다. Kafka append·DB persistence 동시 진행률, 실행 중 Worker peak, API `2.0.0`, migration → Worker → API gate를 포함합니다. CI는 검증된 `demo-dev` tree와 image tag를 단일 release commit으로 `demo-lite`에 게시합니다.
...
- `demo-dev` candidate의 source Demo UI는 `2.2.0`입니다. Kafka append와 동시에 시작하는 persistence summary `1초` polling, 실행 중 Worker readiness `5초` polling과 시작·peak replica 유지가 기준입니다.
...
- Operations Advisor는 readiness, DLQ, 남은 예약, Kafka 적재 수, DB 저장 수의 불일치를 함께 확인해야 합니다. 미확인 이벤트가 남아 있으면 `정상`이 아니라 확인 필요 상태로 표시합니다.
