...
class TestOperationalDocumentation:
    ...
    def test_static_order_dashboard_demo_exists(self):
        ...
        for token in (
            ...
            "const tokenKey = `${baseUrl}|${username}|${password}`",
            "if (!options.forceRefresh && cached && cached.expiresAt > Date.now())",
            "if (!ensuredDemoUsers.has(demoUserKey))",
            "ver.",
            "api",
            "language-toggle",
            "setLanguage",
            ...
            "replayAllDlqEvents",
            "requestDlqReplay",
            "recordReservationSendFailed",
            "pollDlqReplayStatus",
            "dlqReplayComplete",
            "dlqReplayPending",
            "dlqReplayAllButton",
            "dlqReplayAllComplete",
            "/v1/dlq/ingress/replay",
            "DLQ 상세 보기",
            "수동 재처리",
            "전체 수동 재처리",
            "재처리 완료",
            ...
            'value="30000" data-i18n="interval30" selected',
            'value="60000" data-i18n="interval60"',
            "Every 30s",
            "Every 60s",
            "document.querySelector(\"#ops-refresh-interval\").value || 30000",
            "forceRefresh: true",
