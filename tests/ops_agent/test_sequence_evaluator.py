...
def _shift_bundle(bundle: dict, seconds: int, index: int) -> None:
...
def _set_measurement(bundle: dict, latest_lag: int, slope: int) -> None:
...
def _apply_case(sequence: list[dict], case: dict) -> list[dict]:
    ...
    if operation == "set_capture_measurements":
        for bundle, lag, slope in zip(
            ...
            _set_measurement(bundle, lag, slope)
