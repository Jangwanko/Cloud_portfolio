...
def test_demo_lite_keeps_low_resource_topology_with_current_v2_contract() -> None:
    ...
    autoscaling_patch = _read(
        "k8s/gitops/overlays/demo-lite/patches/autoscaling-lite.yaml"
    )
    kafka_patch = _read("k8s/gitops/overlays/demo-lite/patches/kafka-lite.yaml")
    topic_patch = _read(
        "k8s/gitops/overlays/demo-lite/patches/kafka-topic-bootstrap-lite.yaml"
    )
    environment = _read(
        "k8s/gitops/overlays/demo-lite/patches/messaging-env-lite.yaml"
    )
    ...
    assert "size: 4Gi" in postgresql
    for committed_secret in (
        "password: portfolio",
        "repmgrPassword:",
        "adminPassword:",
        "1q2w3e4r",
...
def test_demo_lite_local_and_public_image_boundaries_are_separate() -> None:
    ...
    public_api_gate = _read(
        "k8s/gitops/overlays/demo-lite-k3s/patches/api-generic-v2-k3s.yaml"
    )
    public_environment = _read(
        "k8s/gitops/overlays/demo-lite-k3s/patches/messaging-env-k3s.yaml"
    )

    assert "images:" not in local_overlay
    assert "images:" in public_overlay
    assert "newName: ghcr.io/jangwanko/cloud_portfolio" in public_overlay
    assert "newTag:" in public_overlay
    assert "patches/api-generic-v2-k3s.yaml" in public_overlay
    assert "patches/messaging-env-k3s.yaml" in public_overlay
    assert 'GENERIC_EVENTS_V2_ENABLED\n              value: "true"' in public_api_gate
    assert "APP_VERSION:" in public_environment


def test_demo_lite_rollout_opens_v2_only_after_migration_and_worker() -> None:
    migration = _read("k8s/gitops/base/migration-job.yaml")
    windows = _read("scripts/quick_start_lite.ps1")
    linux = _read("scripts/deploy_lite_k3s.sh")

    assert 'argocd.argoproj.io/sync-wave: "-2"' in migration
    windows_migration = "job/messaging-schema-migration"
    windows_worker = "deployment/worker"
    windows_gate = "GENERIC_EVENTS_V2_ENABLED=true"
    assert windows.index(windows_migration) < windows.index(windows_worker) < windows.index(
        windows_gate
    )
    linux_migration = "job/messaging-schema-migration"
    linux_worker = "deployment/worker"
    linux_gate = "GENERIC_EVENTS_V2_ENABLED=true"
    assert linux.index(linux_migration) < linux.index(linux_worker) < linux.index(
        linux_gate
    )


def test_demo_lite_publication_requires_the_shared_ci_validation_gate() -> None:
    workflow = _read(".github/workflows/ci.yml")
    publish = workflow.split("  publish-demo-lite-image:", 1)[1].split(
        "  publish-master-image:", 1
    )[0]

    assert "github.ref == 'refs/heads/demo-dev'" in publish
    assert "needs: validate" in publish
    assert "ref: ${{ github.sha }}" in publish
    assert "Build and push candidate image" in publish
    assert "Verify published candidate digest" in publish
    assert "docker buildx imagetools create" in publish
    assert "git rev-parse origin/demo-dev" in publish
    assert "git checkout --detach origin/demo-lite" in publish
    assert 'git read-tree --reset -u "$GITHUB_SHA"' in publish
    assert "git push origin HEAD:demo-lite" in publish
    assert "k8s/gitops/overlays/demo-lite-k3s/kustomization.yaml" in publish
    assert "k8s/gitops/overlays/demo-lite-k3s/patches/messaging-env-k3s.yaml" in publish
    assert not (ROOT / ".github/workflows/demo-lite-image.yml").exists()


def test_demo_lite_linux_runtime_secrets_are_generated_or_reused() -> None:
    script = _read("scripts/deploy_lite_k3s.sh")

    assert 'GRAFANA_ADMIN_PASSWORD="${GRAFANA_ADMIN_PASSWORD:-}"' in script
    assert "existing_auth_secret" in script
    assert "existing_grafana_password" in script
    assert "secrets.token_urlsafe(48)" in script
    assert "secrets.token_urlsafe(24)" in script
    assert "1q2w3e4r" not in script
    assert "lite-demo-auth-secret-" not in script
    assert "get secret messaging-postgresql-ha-postgresql" not in script
    assert "postgres_password" not in script
    assert "/opt/bitnami/postgresql/secrets/postgres-password" in script
